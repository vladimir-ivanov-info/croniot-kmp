...
class ParameterTaskDaoImpl : ParameterTaskDao {
    ...
    override fun getByUid(parameterTaskUid: Long, taskType: TaskType): ParameterTask? {
       ...
       /* session.use { sess ->
            ...
            val deviceIdPredicate = cb.equal(root.get<ParameterTask>("taskType"), taskType)
        ...
        session.use { sess ->
            ...
            //getConstraints()
// Map the result to a ParameterTask instance
    ...
    fun getConstraints(parameterTaskId: Long): MutableMap<String, String> {
        val session = ControllerDb.sessionFactory.openSession()
        session.use { sess ->
            val sql = """
